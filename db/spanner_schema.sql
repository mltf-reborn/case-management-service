...
--       --instance=mltf-spanner \
--       --project=mltf-506212 \
--       --ddl="CREATE TABLE \`case\` (case_id STRING(64) NOT NULL, user_id STRING(64) NOT NULL, case_type STRING(32) NOT NULL, case_status STRING(32) NOT NULL, document_url STRING(1024), selfie_url STRING(1024), document_verification_details JSON, selfie_details JSON, kyc_details JSON, external_kyc_details JSON, risk_score FLOAT64, risk_level STRING(32), rejection_reason STRING(1000), remarks STRING(1000), assigned_to STRING(64), created_at TIMESTAMP NOT NULL OPTIONS (allow_commit_timestamp = true), updated_at TIMESTAMP NOT NULL OPTIONS (allow_commit_timestamp = true)) PRIMARY KEY (case_id); CREATE INDEX idx_case_user_id ON \`case\`(user_id); CREATE INDEX idx_case_status ON \`case\`(case_status); CREATE INDEX idx_case_type ON \`case\`(case_type); CREATE INDEX idx_case_created_at ON \`case\`(created_at DESC);"
--
-- To add this column to an EXISTING table, run the ALTER TABLE statement:
--
--   gcloud spanner databases ddl update mortgage_db \
--       --instance=mltf-spanner \
--       --project=mltf-506212 \
--       --ddl="ALTER TABLE \`case\` ADD COLUMN external_kyc_details JSON;"
--
-- ------------------------------------------------------------------------------
...
CREATE TABLE `case` (
    ...
    external_kyc_details JSON,             -- Raw KYC details received from external provider at case creation
